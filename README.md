...
Traffic is measured with **Cloudflare Web Analytics** — **cookieless**, sets no
browser storage, does no cross-site tracking or fingerprinting, and collects **no
personal data**, so it fits the app's privacy-first, kid-friendly stance (the
onboarding copy discloses that visits are counted anonymously).

It is **off unless a token is provided.** To enable it: create a Web Analytics site
in the Cloudflare dashboard, copy the beacon **token**, and add it as a repository
**Actions secret** named `CF_BEACON_TOKEN`. The deploy build then injects Cloudflare's
beacon into the production HTML (see `cloudflareAnalytics()` in
[`vite.config.ts`](vite.config.ts)); dev and tests never load it, and with no token
the beacon is simply omitted.

## Social preview & icons

- **`public/og-image.png`** (1200×630) is the Open Graph / Twitter card image shown
  when the link is shared; the tags live in [`index.html`](index.html) with absolute
  `https://worldexplorer.cc/…` URLs. Regenerate it by rendering a 1200×630 HTML
  template to PNG (it's a committed static asset, not built in CI).
- **Home-screen / PWA icons** are generated by
  [`scripts/generate-icons.mjs`](scripts/generate-icons.mjs) (`npm run icons`, also run
  before `dev`/`build`) — a dependency-free encoder that draws the globe from
  `public/favicon.svg`, so the installed app and Add-to-Home-Screen icon are branded.
